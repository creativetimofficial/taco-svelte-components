...
export function getTextareaClass(variant, color, size, error, success, resize) {
  const structureClass = {
    base: `peer h-full w-full min-h-[100px] bg-transparent text-blue-gray-700 font-normal font-sans outline outline-0 transition-all focus:outline-0 disabled:bg-blue-gray-50 disabled:border-0 placeholder-shown:border-blue-gray-200 ${
...
export function getLabelClass(variant, color, size, error, success) {
  const structureClass = {
    ...
    variants: {
      ...
      standard: {
        base: "after:border-b-2 after:-bottom-0 peer-focus:after:scale-x-100 left-0 after:absolute after:scale-x-0 after:transition-transform after:duration-300 after:w-full text-[11px] peer-focus:text-[11px] peer-placeholder-shown:text-sm -top-1.5",
      ...
      static: {
        base: "after:border-b-2 after:-bottom-1 peer-focus:after:scale-x-100 left-0 after:absolute after:scale-x-0 after:transition-transform after:duration-300 after:w-full text-sm peer-focus:text-sm -top-2.5",
        color: `text-blue-gray-500 ${states.peerFocus.text[color]} ${states.after.border[color]} ${states.peerFocus.after.border[color]}`,
        error:
          "text-red-500 peer-focus:text-red-500 peer-placeholder-shown:text-red-500 after:border-red-500 peer-focus:after:border-red-500",
        success:
